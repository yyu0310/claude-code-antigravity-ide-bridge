...
| 需求 | 说明 |
|---|---|
| **Gemini Pro 订阅** | 获取 AG 内置的 Claude 免费额度；已有 Anthropic API Key 者可跳过 |
...
Google AI Pro 订阅用户另有每月 **1,000 AI Credits**。开启 **Settings → Models → Enable Overages** 后，session 配额耗尽时会自动消耗 Credits。Credits 与 Claude token 的换算比例未公开，实际能多用多少不确定。
...
确认 symlink：
```bash
ls -la /你的工作区/GEMINI.md
# 正确输出：GEMINI.md -> /你的工作区/CLAUDE.md
```
...
两者格式不兼容，`scripts/memory_sync.py` 负责双向转换。
...
目录名称是工作区路径把 `/` 换成 `-` 的结果。例如 `/Users/alice/my-project` → `-Users-alice-my-project`。
...
Claude Code 读取 `~/.claude/commands/` 的 slash command。
Antigravity IDE 读取 `~/.gemini/config/plugins/<plugin-name>/skills/` 的 skill。

`scripts/skill_sync_setup.py` 建立 symlink，让两个 IDE 读同一份来源文件，无重复、不漂移。

#### 前置条件：YAML frontmatter

每个 skill 的 `.md` 文件顶部需要有 frontmatter：
...
Antigravity IDE 通过 frontmatter 注册 skill 名称和描述；Claude Code 也会读取，但不强制要求。
...
**Step 2** — 为没有 frontmatter 的 skill 文件手动加上后执行：
